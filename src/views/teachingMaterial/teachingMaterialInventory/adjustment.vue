<template>
  <div id="adjustment" ref="adjustment">
    <search-bar
      :datas="searchConfig"
      :config="btnConfig"
  ...
  export default {
    ...
    methods: {
      ...
      dblclick(row, column, cell, event) {
        if(row.state === 2) {
          this.$router.push({ path: '/teachingmaterial/teaching-material-inventory/adjustment-detail', query: { type: 'detail', id: row.id} })
        }else {
          this.$router.push({ path: '/teachingmaterial/teaching-material-inventory/adjustment-detail', query: { type: 'edit', id: row.id} })

        }
      },
      // 计算表格高度
      getTableHeight() {
        const assetAccept = document.getElementById('adjustment').clientHeight
      ...
      // 点击生成采购计划
      addData() {
        this.$router.push({ path: '/teachingmaterial/teaching-material-inventory/adjustment-detail', query: { type: 'add' } })
      },
      // 查询数据列表
      getList() {
        let param = {
          adjustmentDateEnd: this.searchConfig.data.adjustmentDate[1],
          adjustmentDateStart: this.searchConfig.data.adjustmentDate[0],
          repertoryAdjustmentCode: this.searchConfig.data.repertoryadjustmentCode,
          state: this.searchConfig.data.state,
          pageNum: this.tableConfig.pageNum,
          pageSize: this.tableConfig.pageSize
        };
        this.tableConfig.loading = true;
        getAdjustment(param).then((Result) => {
