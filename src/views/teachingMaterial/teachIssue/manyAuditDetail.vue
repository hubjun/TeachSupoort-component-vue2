<template>
  <div id="manyAuditDetail" ref="manyAuditDetail" class="com-detail-wrap">
    <div class="detail-item">
      <div class="detail-head">
        <div class="tit">基本信息</div>
        <div class="btns">
          <el-button class="com-btn" type="info" @click="cancel" plain>返回</el-button>
          <el-button class="com-btn" type="primary" @click="audit" plain>审核</el-button>
          <el-button class="com-btn" type="danger" @click="reject" plain>驳回</el-button>
        </div>
      </div>
...
import { getBookGrantDeclare, auditBookGrantDeclare } from './api'
export default {
  components: {
    FormList,
    EditTable,
  ...
  methods: {
    ...
    getTableHeight() {
      const manyAuditDetail = document.getElementById('manyAuditDetail')
        .clientHeight
      let detailItem = document
        .getElementById('manyAuditDetail')
      ...
      this.Form.tableHeight =
        manyAuditDetail - detailItem - detailHead  - 60 - 22 // 60为table-box的内边距，两个20位margin-bottom
    },
    cancel() {
      this.$router.go(-1)
    },
    audit() {
      let params = {
        ...
        step: this.formData.state,
        audit: 1
      }
      auditBookGrantDeclare(params).then(res => {
        if(res.status) {
         params.step === 2 ? this.$success('申报单核准') : this.$success('申报单审批')
          this.$router.go(-1)
        }
      })
    },
