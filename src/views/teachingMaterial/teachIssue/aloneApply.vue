<template>
  <div id="aloneApply" ref="aloneApply">
    <search-bar
      :datas="searchConfig"
      :config="btnConfig"
        ...
        >
          <template slot-scope="scope">
            <div v-if="item.prop === 'state'">
              <div v-if="scope.row[item.prop] === 5"
                class="com-status-unusual"
              >已驳回</div>
              <div
              v-else
...
export default {
  ...
  methods: {
    getTableHeight() {
      const aloneApply = document.getElementById('aloneApply').clientHeight
      let searchBox = document
        .getElementById('aloneApply')
      ...
      this.tableConfig.tableHeight =
        aloneApply - searchBox - pagination - 40 - 10 // 40为table-box的内边距，两个20位margin-bottom
    },
    handleSearch() {
