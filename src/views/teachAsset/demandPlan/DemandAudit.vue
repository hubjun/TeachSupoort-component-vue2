<template>
  <div id="DemandAudit" ref="DemandAudit">
    <search-bar
      :datas="searchConfig"
      :config="btnConfig"
  ...
  export default {
    ...
    methods: {
      ...
      dblclick(row, column, cell, event) {
        if(row.checkState === 1) {
          this.$router.push({ path: '/teachasset/demand-plan/demand-audit-detail', query: { type: 'edit', id: row.id } })
        }else {
          this.$router.push({ path: '/teachasset/demand-plan/demand-audit-detail', query: { id: row.id } })
        }
      },
      // 计算表格高度
      getTableHeight() {
        const assetAccept = document.getElementById('DemandAudit').clientHeight
        let searchBox = document
          .getElementById('DemandAudit')
