...
<script>
  ...
  export default {
        ...
        searchConfig: {
          ...
          config: [
            ...
            },
            {
              ...
              labelWidth: '0',
              prop: 'Name',
              placeholder: '请输入部门编码/名称/拼音码/五笔码',
              type: 'text'
            }
          ]
        },
    ...
    methods: {
      ...
      // 点击编辑
      editclick(id) {
        this.$router.push({ path: '/teachasset/base-dict/depart-detail', query: { type: 'edit', id: id } })
      },
      // 双击表格某列
      dblclick(row, column, cell, event) {
        this.$router.push({ path: '/teachasset/base-dict/depart-detail', query: { type: 'edit', id: row.id } })
      },
      // 计算表格高度
      getTableHeight() {
        const assetAccept = document.getElementById('Depart').clientHeight
      ...
      // 点击新增
      addData() {
        this.$router.push({ path: '/teachasset/base-dict/depart-detail', query: { type: 'add' } })
      },
      // 点击批量启用
      multiOpen() {
      ...
      getList() {
        let param = {
          ...
          state: this.searchConfig.data.DepartStatus,
