<template>
  <div id="parameterManageDetail" ref="parameterManageDetail" class="com-detail-wrap">
...
export default {
  ...
  watch: {
    ...
    active() {
      ...
      this.active === 6 && this.getzcclycList(this.formData.id)
    }
  },
  computed: {
    formConfig() {
  ...
  data() {
    let lifeValidator = (rule, value, callback) => {
      if (!value) {
        return callback(new Error('使用年限不能为空'))
      }
      if (!/^[1-9]\d*$/.test(value)) {
    ...
    return {
      ...
      listRules: {
        ...
        // currentPrice: [
        //   { required: true, message: '资产净值不能为空' },
        //   { type: 'number', message: '资产净值必须为数字值' }
        // ]
      },
      tabList: [
        '资产领用',
  ...
  methods: {
    ...
    getDetail() {
      ...
      this.assetQrcodes.url = obj.assetQrcodes && obj.assetQrcodes.url
