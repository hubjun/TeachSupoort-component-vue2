<template>
  <div id="parameterManage" ref="parameterManage">
    <search-bar
      :datas="searchConfig"
      :config="btnConfig"
...
import { getAssetsRecord } from './api'
...
export default {
  ...
  methods: {
    getTableHeight() {
      const parameterManage = document.getElementById('parameterManage')
        .clientHeight
      let searchBox = document
        .getElementById('parameterManage')
      ...
      this.tableConfig.tableHeight =
        parameterManage - searchBox - pagination - 40 - 10 // 40为table-box的内边距，两个20位margin-bottom
    },
    handleSearch() {
    ...
    getList() {
      let params = {
        ...
        user_name: this.searchConfig.data.user_name
      }
      getAssetsRecord(params).then(res => {
        if (res.status) {
