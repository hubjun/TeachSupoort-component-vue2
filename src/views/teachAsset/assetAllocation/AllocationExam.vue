<template>
  <div id="AllocationExam" ref="AllocationExam">
    <search-bar
        ...
        >
          <template slot-scope="scope">
            <div v-if="item.prop === 'allocateStatus'">
              <div v-if="scope.row[item.prop] === 3" class="com-status-info">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        detailPath: '/teachasset/asset-allocation/allocation-exam-detail',
        // 搜索按钮加载
        isLoading: false,
        // 搜索栏配置
        searchConfig: {
    ...
    methods: {
      // 动态设置表格高度
      getTableHeight() {
        const assetAccept = document.getElementById('AllocationExam').clientHeight
        let searchBox = document
          .getElementById('AllocationExam')
