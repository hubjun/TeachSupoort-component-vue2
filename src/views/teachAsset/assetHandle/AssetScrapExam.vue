<template>
  <div id="AssetScrapExam" ref="AssetScrapExam">
    <search-bar
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        detailPath: '/teachasset/asset-handle/asset-scrap-exam-detail',
        // 搜索按钮加载
        isLoading: false,
        // 搜索栏配置
        searchConfig: {
    ...
    methods: {
      // 动态设置表格高度
      getTableHeight() {
        const assetAccept = document.getElementById('AssetScrapExam').clientHeight
        let searchBox = document
          .getElementById('AssetScrapExam')
      ...
      // 双击事件
      dblclick(row) {
        localStorage.setItem('AssetScrapExam', JSON.stringify(row))
        this.$router.push({
          path: this.detailPath,
          query: {
            type: 'detail',
